...
export default function BoardPage() {
  ...
  async function moveBook(book: Book, newStatus: BookStatus, clientY: number) {
    ...
    setBooks((prev) =>
      prev ? prev.map((b) => (b.trello_id === book.trello_id ? optimistic : b)) : prev
    );
    ...
    <div className="space-y-3 -mx-4 sm:mx-0">
      <div className="flex items-center justify-between px-4 sm:px-0">
        <div>
          ...
          <p className="text-sm text-stone-500">Drag books between columns to update status.</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-4 px-4 sm:px-0">
        {COLUMNS.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className="flex-none w-64 bg-stone-100 rounded-xl border border-stone-200 flex flex-col max-h-[calc(100vh-220px)]"
            >
                ...
                {list.map((b) => {
                  return (
                    <div
                      ...
                      className={
                        "flex gap-2 bg-white border border-stone-200 rounded-lg p-2 shadow-sm cursor-grab active:cursor-grabbing " +
